...
class FastaParser:
    ...
    def __iter__(self):
        with read_gz_file(self.file) as fastaFile:
...
def txome_to_orfs(mrnaFileIn, cdsFileOut, protFileOut, translationTable):
    '''
...
# Define functions
def generate_sequence_length_index(fastaFile, lengthFile):
    '''
    ...
    with open(lengthFile, "wb") as fileOut:
...
def remove_sequence_from_fasta(fastaFile, sequenceIDs, outputFile, force=False):
    ...
    with read_gz_file(fastaFile) as fileIn, open(outputFile, "w") as fileOut:
        records = SeqIO.parse(fileIn, "fasta")
        for record in records:
            seqID = record.id
