...
def cmain(args, locations):
    ...
    unbinnedIDs = get_unbinned_sequence_ids(bingeResults.binned, eliminations, transcriptRecords)
    if args.debug:
        print(f"# There are {len(unbinnedIDs)} unbinned sequences for external clustering")
    
    if len(unbinnedIDs) == 1:
        bingeResults.unbinned = { 0: list(unbinnedIDs)[0] }
    elif len(unbinnedIDs) > 0:
        bingeResults.unbinned = cluster_unbinned_sequences(unbinnedIDs, transcriptRecords,
                                                           args, locations.tmpDir, runDir)
    else:
        bingeResults.unbinned = {} # blank to signal an absence of results
...
def vmain(args, locations):
    ...
    if os.path.exists(clusterFile):
        bingeResults = BINge_Results(clusterFile)
        bingeResults.parse_binge_clusters()
